...
void PWSupportCheckIsPWHTTPRequestCallback(bool success, const char[] error, System2HTTPRequest request, System2HTTPResponse response, HTTPRequestMethod method)
{
    ...
    static char result[PLATFORM_LINE_LENGTH], IsOnline[16];
    ...
    snap.GetKey(0, string(gClientData[client].Steam64));
    data.GetString(gClientData[client].Steam64, string(IsOnline));
...
void PWSupportConnectRequest(int client)
{
    ...
    FormatEx(
        ...
        \n    \"zhCN\"              :           \"%s\"      \
        \n}",
        timestamp, COMMUNITY_ID, gServerData.IPAddr, gClientData[client].Steam64, gClientData[client].PerfectWorld ? "true" : "false"
    );
